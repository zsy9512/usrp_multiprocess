...
用法:
  ...
  python tools/analyze_repeat_capture.py capture/low_snr_v2 --plot
  python tools/analyze_repeat_capture.py capture/low_snr_v2 --save-plot plots
"""
...
def analyze_capture(prefix, num_frames=40, pss_ptm=2.5, pss_pts=1.0,
    ...
    # 帧间隔从 meta 读取 (v1: 5ms/3ms, v2: 30ms/5ms)
    ...
    # STF/RS 长度也从 meta 读 (v1: 64/32, v2: 128/64)
    ...
    # 检测 v2 (Polar 编码)
    is_v2 = stf_len_sym > 64 or rs_len_sym > 32

    # TX 时序定位
    rng = np.random.RandomState(42)
    if is_v2:
        # v2: rng.rand(K_POLAR) 而不是 rng.randint
        from tools.loopback_capture_v2 import build_frame_v2, rrc_pulse
        import math
        FROZEN_MASK = np.load(os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            'deploy', 'matrices', 'A.npy')).squeeze()
        def _polar_enc(u):
            cw = u.copy().ravel()
        ...
        coded = _polar_enc(u)
        tx_iq_ref = rrc_pulse(
            build_frame_v2(coded, 0, stf_syms, pss_syms, rs_syms),
...
def _oracle_measure_one_frame(iq, expected_offset, frame_iq_len, rng, frame_id,
    ...
    独立于 PSS+RS 同步链。
    use_polar=True: 模拟 Polar 编码负载 (v2 capture), RNG 调用匹配 loopback_capture_v2.
    Returns dict 或 None (如果找不到帧或信号过弱).
    """
    from tools.sync_sweep import build_custom_frame
    if use_polar:
        # v2: 匹配 loopback_capture_v2.py 的 RNG 调用序列
        # rng.rand(K_POLAR) -> polar_encode -> build_frame_v2
        ...
        from tools.loopback_capture_v2 import build_frame_v2, rrc_pulse
        frame_syms = build_frame_v2(coded, frame_id, stf_syms, pss_syms, rs_syms)
