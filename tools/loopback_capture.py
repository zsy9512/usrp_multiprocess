...
默认帧配置:
  ...
  - 帧间间隔 (不同 frame_id) = 30 ms, 重复帧间隔 = 5 ms
...
用法:
  python tools/loopback_capture.py --serial 320F33F --gain-tx 60 --gain-rx 30 -o capture/test
"""
...
def main():
    ...
    p.add_argument('-o', '--output', default='capture/test')
    args = p.parse_args()
    ...
    def tx_thread():
        ...
        for f in range(args.num_frames):
            ...
            frame_syms = build_frame(coded, f, stf_syms, pss_syms, rs_syms)
