...
def decode_capture(prefix, num_frames=100, sgnn_model=None, sgnn_graph=None,
    ...
    # 重建 frame0 TX 波形
    def _reconstruct_frame(info, fid):
        ...
        frame_syms = build_frame(coded, fid, stf_syms, pss_syms, rs_syms)
        return rrc_pulse(frame_syms, RRC, SPS).astype(np.complex64)

    tx_ref0 = _reconstruct_frame(info0, 0)
...
def main():
    ...
    p.add_argument('--sgnn', dest='sgnn', action='store_true', default=True,
                   help='启用 SGNN 译码 (默认启用)')
    p.add_argument('--no-sgnn', dest='sgnn', action='store_false',
                   help='禁用 SGNN, 仅运行 Hard Inverse baseline')
