...
本系统基于USRP硬件，支持DQPSK调制解调的多进程收发、同步、实时处理和可视化。自v2.0.0起，系统集成了CRC-16-CCITT数据验证和快速丢弃功能，提供完整的帧级错误检测能力。硬件时钟源同步方案已完善，单收单发模式也能稳定运行，误码率可达5e-3以下。系统架构灵活，支持仿真、硬件、自收自发三种模式，适合科研、教学和原型开发。
...
| **硬件USRP模式** | <5e-3            | <2%       | ✅ 已跑通       | 需要搭配硬件时钟源同步 |
...
### v2.0.0 (当前版本)
- ✅ **CRC-16-CCITT集成**：完整的帧级数据验证和快速丢弃功能
- ✅ **帧结构升级**：PSS+SSS+RS+FrameIndex+CRC-16+Data完整结构
- ✅ **错误检测增强**：CRC校验失败自动丢弃，提升系统可靠性
- ✅ **所有模式兼容**：仿真、硬件、自收自发模式均支持CRC功能
- ✅ **性能验证完成**：BER性能稳定，CRC丢弃率控制在合理范围内
...
### v2.0.0 (2025-09-25)
- ✅ **CRC-16-CCITT正式集成**：完整的帧级数据完整性验证系统
- ✅ **帧结构全面升级**：新增CRC-16字段，总帧长1304比特
- ✅ **快速丢弃机制**：CRC校验失败帧自动丢弃，提升系统效率
- ✅ **多文件协同更新**：processing_program.py、simple_simulation_test.py、analyze_rxdata.py全部适配
- ✅ **BER计算优化**：正确跳过帧序号和CRC字段，只计算数据比特BER
- ✅ **性能指标扩展**：新增CRC丢弃率监控，系统可靠性显著提升
- ✅ **跨模式兼容性**：仿真、硬件、自收自发三种模式均完美支持CRC功能
- ✅ **完整性验证**：所有功能经过全面测试，系统稳定可靠
